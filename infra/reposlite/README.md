...
# Raw file-storage proxy
wget -S -O /dev/null "http://localhost:8080/file-storage/sysroot-ohos-aarch64-6.0.2.640-02.tar.gz"
```

### `releases` 与 `distributions`（Gradle zip）

- `**releases**`：聚合 Maven 上游，供 `proxy.init.gradle`（Gradle Maven 镜像段）等使用（`/releases/...`）。
  - 其中 `maven.eazytec-cloud.com/nexus/repository/maven-public/` 额外允许 `.tar.gz` / `.tgz` / `.zip`，用于代理 `kotlin-native-prebuilt` 这类归档产物（例如 `...-macos-aarch64.tar.gz`）。
...
- `**file-storage**`：专门代理 `https://maven.eazytec-cloud.com/nexus/repository/file-storage/`（raw 仓库），用于缓存 `sysroot` 等归档文件。本地 URL：`/file-storage/<filename>`（例如 `sysroot-ohos-aarch64-6.0.2.640-02.tar.gz`）。
