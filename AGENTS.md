...
8. **Artifacts** — Archive **`llvm/packages`** (and naming/metadata) consistent with downstream **cross-copy** / **ohos-test-build** jobs in the same file; upload via local server action where configured. **cross-copy** runs **`platform_package.sh`** with **`clang-dev-windows-x86_64.tar.gz`** from the Linux `packages/` tree (alongside Darwin tars), overlays host libcxx-ndk tarballs from Linux packages into each merged host tree, then publishes **`llvm-<major>-<konan_triple>-dev-<id>-run<github_run_id>`** artifacts on the LAN server. **Linux/macOS** outputs are **gzip tarballs**; **Windows** output is a **zip**. Inside each artifact, the first member is exactly one top-level directory **`llvm-<major>-<konan_triple>-dev-<id>/`** (Konan **`~/.konan/dependencies`** layout; no **`-run…`** in the folder name). **Darwin**, **Windows**, and **Linux** final artifacts are all flattened host trees: **`bin/`**, **`lib/`**, etc. live directly under that top-level directory (not under **`clang_*-*-<timestamp>/`**). Workflow inputs **`final_llvm_artifact_version`** (dev id, default **200**) and **`llvm_major_version`** (default **19**) control that stem; not **`llvm-merged-ohos-*`** / not raw outer **`llvm-packages-*.tar`** in **ohos-test-build**.
