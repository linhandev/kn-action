...
**Workflow behavior** — [`.github/workflows/build-kotlin.yml`](.github/workflows/build-kotlin.yml) **Setup environment** (Windows only) copies **`id_ed25519`**, **`id_ed25519.pub`**, and relevant **`known_hosts`** lines from **`NetworkService\.ssh`** into the job’s **`$HOME/.ssh`** only when those paths differ (if **`$HOME`** is already the service profile, the copy is skipped). It then ensures **`gitcode.com`** is in **`known_hosts`** (via **`ssh-keyscan`** if needed). If that step fails with missing key or host key, fix the **Network Service** `.ssh` layout on the host first.
